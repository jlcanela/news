...
import { Article, ArticleRepository, Topic } from "./ArticleRepositories";
...
interface SelectArticlesProps {
  ...
  onSelect: (arg0: SelectArticlesContext) => void;
}
...
export interface MailingStatusProps {
  goHome: () => void;
}

function MailingStatus(props: MailingStatusProps) {
  return (
    <Paper sx={{ p: 3 }}>
      <MainTitle>Mailing status</MainTitle>
      <SubTitle>Your mailing have been successfully registered.</SubTitle>
      <Button onClick={() => props.goHome()}>Go to Home</Button>
    </Paper>
  );
}
...
export function Mailing(props: MailingProps) {
  ...
  return (
    <>
      ...
      <Show when={currentStep() == 0}>
        <SelectArticles
          articleRepository={props.articleRepository}
          initialContext={sac}
          onSelect={onSelect}
        />
      </Show>
      ...
      <Show when={currentStep() == 3}>
        <MailingStatus goHome={props.goHome} />
      </Show>
    </>
  );
}
